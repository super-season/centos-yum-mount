...
from TagProc import template_c12_liu
...
class TagLogicPrc(object):
    ...
    def b2_base_gen(self, display_fields):
        # 生成tag/dld/mid表，含必显字段,得到member_key的键
        ...
        for tb_obj in tb_obj_list:
            ...
            for table_relation in self.table_relation[tb_obj]:
                ...
                if table_relation[0] in tb_set:
                    # 更新左表表名
                    tb_left = table_relation[0]   #[['test_product_detail', [['product_ID', 'product_ID']]]]
                else:
                    # 拼接关联关系,0代表第一个，1代表第二个，['product_ID', 'product_ID']，r1=test_sales_summary.product_ID = test_product_detail.product_ID
                    r1 = [(tb_left + '.' + r[0] + ' = ' + table_relation[0] + '.' + r[1]) for r in table_relation[1]]
                    sql += '\n' + join_str + ' join ' + table_relation[0] + ' \non ' + ' and '.join(r1)
                    #sql = left(默认值) join    test_product_detail        on   and test_sales_summary.product_ID = test_product_detail.product_ID
                    # 更新左表表名
    ...
    def b_2001_1_tag_type1_where(self, base_conf):    #新增，两个表内连接
        # 类型1 tag表 where条件
        self._wr_comment(sys._getframe().f_code.co_name)

        base_conf['u_tag'] = self.tag_tb_name
        self._i_02_1_type1_where(base_conf)
    ...
    def _i_02_1_type1_where(self, base_conf):       #新添加，连接两个表
        # 类型1 2表 where条件
        # 如果是t_tag表则字段前面加t_u_
        tu_tag_field_name = ('t_u_' if base_conf['u_tag'] == self.tag_tb_name else '') + base_conf['tag_field_name']
        # 拼接关联关系

        join_table1 = self._table_relation(base_conf['u_tag'], base_conf['table_join1'])\
            if 'table_join1' in base_conf.keys() else ''
        join_table2 = self._table_relation(base_conf['table_join1'], base_conf['table_join2']) \
            if 'table_join2' in base_conf.keys() else ''
        template_dict = {
            'tag_cn_name': base_conf['tag_cn_name'],
            'tu_tag_field_name': tu_tag_field_name,
            'tag_field_type': base_conf['tag_field_type'],
            'tag_field_default': base_conf['tag_field_default'],
            'u_tag': base_conf['u_tag'],
            'join_table1': join_table1,
            'join_table2': join_table2,
            'tag_value': base_conf['tag_value'],
            'where_str': base_conf['where_str'],
        }
        sql = template.tl_i_03_1_type1_where_template.substitute(template_dict)
        self._wr_sql(sql)
...
#   """
    ...
    c13_config = {'file_path': template_c12_liu.TAG_FILE_PATH,
                  'custom_id': template_c12_liu.CUSTOM_ID,
                  'db_config': template_c12_liu.DB_CONFIG,
                  'tag_tb_name': template_c12_liu.TAG_TB_NAME,
                  'dld_tb_name': template_c12_liu.DLD_TB_NAME,
                  'mid_tb_name': template_c12_liu.MID_TB_NAME,
                  'member_key': template_c12_liu.MEMBER_KEY,
                  'table_relation': template_c12_liu.TABLE_RELATION,
                  }
    t = TagLogicPrc(c13_config)
    print(t._table_relation('u_tag_12', ['table2']))
    print(t._table_relation('u_tag_12', ['table2'], 'inner'))
    print("Finish")
